...
def compile_model_cnn(geneparam, nb_classes, input_shape):
    ...
    logging.info("Architecture:%d, %s, %s, %s, %d" % (nb_neurons, activationL, activationR, optimizer, nb_layers))
    ...
    depth = nb_neurons
    nb_dense_block = nb_layers
    growth_rate = 12
    nb_filter = -1
    dropout_rate = 0.0  # 0.0 for data augmentation

    model = densenet.DenseNet(img_dim, classes=nb_classes, depth=depth, nb_dense_block=nb_dense_block,
                              growth_rate=growth_rate, nb_filter=nb_filter, dropout_rate=dropout_rate, weights=None, activation_convL=activationL, activation_convR=activationR )
    print("Model created")
###########################################################################################################################
...
def train_and_score(geneparam, dataset):
    ...
    return score[1], trainable_count # 1 is accuracy. 0 is loss.
