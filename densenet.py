...
def f(x):
    #print('new function L= %s R= %s' %(activationLeft,activationRight))
    if (activationLeft=='elu'):
        outleft = tf.nn.elu(x)
    elif (activationLeft=='relu'):
        outleft=tf.nn.relu(x)
    elif (activationLeft=='tanh'):
        outleft=tf.nn.tanh(x)
    elif (activationLeft=='sigmoid'):
        outleft=tf.nn.sigmoid(x)
    elif (activationLeft == 'swish'):
        outleft = tf.nn.sigmoid(x) * x
    elif (activationLeft == 'selu'):
        outleft = tf.nn.selu(x)
    else:
        outleft = tf.nn.relu(x)


    if (activationRight == 'elu'):
        outright = tf.nn.elu(x)
    elif (activationRight == 'relu'):
        outright = tf.nn.relu(x)
    elif (activationRight == 'tanh'):
        outright = tf.nn.tanh(x)
    elif (activationRight == 'sigmoid'):
        outright = tf.nn.sigmoid(x)
    elif (activationRight == 'swish'):
        outright = tf.nn.sigmoid(x) * x
    elif (activationRight == 'selu'):
        outright = tf.nn.selu(x)
    else:
        outright = tf.nn.relu(x)

    return tf.where(x >= 0.0, outright,outleft)
...
def DenseNet(input_shape=None, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=-1, nb_layers_per_block=-1,
             ...
             include_top=True, weights=None, activation_convL=None, activation_convR=None, input_tensor=None,
    ...
    '''Instantiate the DenseNet architecture,
        ...
        '''
    get_custom_objects().update({'f': Activation(f)})

    global activationLeft
    activationLeft = activation_convL

    global activationRight
    activationRight = activation_convR
...
def __transition_up_block(ip, nb_filters, type='deconv', weight_decay=1E-4):
    ...
    if type == 'upsampling':
        ...
        x = Conv2D(nb_filters, (3, 3), activation='f', padding='same', kernel_regularizer=l2(weight_decay),
                   use_bias=False, kernel_initializer='he_normal')(ip)
        x = SubPixelUpscaling(scale_factor=2)(x)
        x = Conv2D(nb_filters, (3, 3), activation='f', padding='same', kernel_regularizer=l2(weight_decay),
                   use_bias=False, kernel_initializer='he_normal')(x)
    else:
        x = Conv2DTranspose(nb_filters, (3, 3), activation='f', padding='same', strides=(2, 2),
