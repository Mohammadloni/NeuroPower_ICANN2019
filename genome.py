...
class Genome():
    ...
    def update_hash(self):
        ...
        """
        genh = str(self.geneparam['nb_neurons']) + self.geneparam['activationL'] + self.geneparam['activationR'] \
    ...
    def train(self, trainingset):
        ...
        if self.accuracy == 0.0: #don't bother retraining ones we already trained 
            self.accuracy, self.param = train_and_score(self.geneparam, trainingset)
    ...
    def print_genome(self):
        ...
        logging.info("parameter: %.2f%%" % (self.param ))
        ...
        genprint = str(self.param) + str('_') + str(self.geneparam['nb_neurons']) + str('_') + self.geneparam['activationL'] + str('_') \
                   + str(self.generation) + str('_') + self.geneparam['activationR'] +  str('_') + str(self.accuracy) +  str('_') \
                   + str(self.geneparam['nb_layers']) + str('_') + self.geneparam['optimizer'] 
    ...
    def print_genome_ma(self):
        """Print out a genome."""
        logging.info(self.geneparam)
        logging.info("Parameter: %d Acc: %.2f%% UniID: %d Mom and Dad: %d %d Gen: %d" % (self.param, self.accuracy * 100, self.u_ID, self.parents[0], self.parents[1], self.generation))
        logging.info("Hash: %s" % self.hash)    